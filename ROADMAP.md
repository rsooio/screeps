...
- 验收：集成报告曲线中"deliver 执行者数"在生产结束后回落（而非等能量满）✅ 实测生效（tick 401 deliver=1 vs 基线 5）
...
- 验收：升级速率对比基线下降 ✅ 实测 ctrlP 541=129 vs 基线 700=7（~18 倍）

### 1.3 矿工基础设施（专用采集节点）✅ 已完成

- 矿工 = 基础设施（固定产能节点）：无 CARRY 采集掉落能量，deliver 拾取（源侧动态扩展 pickup）
- 部署是投资决策（高成本低数量）：门槛经**参数扫描实验**确定（scripts/param-sweep.ts，两轮共 15 组真实引擎实验）——`minWorkers=1, energyThreshold=300`
  - 能量门槛 <300 灾难性（矿工挤占劳动力，升级停滞，w0/t200 ctrlP=0）
  - minWorkers=0 冷启动缺陷（保底普通被挤，spawnE 储备 61 vs w1 的 147）
